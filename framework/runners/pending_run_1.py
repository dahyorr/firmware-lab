...
entries = []
with open('/root/pending_group_1.txt') as fh:
    for line in fh:
        vendor, path = line.strip().split('|', 1)
        entries.append((vendor, Path(path)))

print(f"[*] Pending-focused run: {len(entries)} images (first attempt, no cache bypass needed)")
summaries = []
