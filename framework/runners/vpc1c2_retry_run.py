...
entries = []
with open('/root/vpc1c2_retry_failed.txt') as fh:
    for line in fh:
        vendor, path, old_iid, old_status = line.strip().split('|', 3)
        entries.append((vendor, Path(path), old_iid, old_status))

print(f"[*] VPC-1 pane c round 2 retry run (force fresh, no cache, watchdog-protected): {len(entries)} images")
summaries = []
...
print("\n[*] VPC-1 PANE C ROUND 2 RETRY RUN COMPLETE")
