...
entries = []
with open('/root/vpc1a2_retry_failed.txt') as fh:
    for line in fh:
        vendor, path, old_iid, old_status = line.strip().split('|', 3)
        entries.append((vendor, Path(path), old_iid, old_status))

print(f"[*] VPC-1 pane a round 2 retry run (force fresh, no cache, watchdog-protected): {len(entries)} images")
summaries = []
...
print("\n[*] VPC-1 PANE A ROUND 2 RETRY RUN COMPLETE")
